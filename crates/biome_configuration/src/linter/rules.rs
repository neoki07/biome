...
pub struct Nursery {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub no_declaration_block_shorthand_property_overrides:
        Option<RuleConfiguration<NoDeclarationBlockShorthandPropertyOverrides>>,
...
impl Nursery {
    ...
    pub(crate) const GROUP_RULES: &'static [&'static str] = &[
        ...
        "noCssEmptyBlock",
        "noDeclarationBlockShorthandPropertyOverrides",
        "noDoneCallback",
    ...
    pub(crate) fn get_enabled_rules(&self) -> IndexSet<RuleFilter> {
        ...
        if let Some(rule) = self
            .no_declaration_block_shorthand_property_overrides
            .as_ref()
        {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[3]));
    ...
    pub(crate) fn get_rule_configuration(
        ...
        match rule_name {
            ...
            "noCssEmptyBlock" => self
                ...
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "noDeclarationBlockShorthandPropertyOverrides" => self
                .no_declaration_block_shorthand_property_overrides
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "noDoneCallback" => self
    ...
    pub(crate) fn set_severity(&mut self, rule_name: &str, severity: RulePlainConfiguration) {
        match rule_name {
            ...
            "noDeclarationBlockShorthandPropertyOverrides" => {
                if let Some(rule_conf) = &mut self.no_declaration_block_shorthand_property_overrides
                {
                    rule_conf.set_level(severity);
                }
            }
